...
def average_data(algorithm="", dataset="", goal="", time="", times=10):
    test_acc = get_all_results_for_one_algo(algorithm, dataset, goal, time, times)
...
def get_all_results_for_one_algo(algorithm="", dataset="", goal="", time="", times=10):
    test_acc = []
    algorithms_list = [algorithm] * times
    for i in range(times):
        file_name = dataset + "_" + algorithms_list[i] + time + "_" + goal + "_" + str(i)
